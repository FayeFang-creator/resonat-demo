...
def _card_from_candidate(candidate: dict, source: str) -> dict | None:
    ...
    enriched = catalog.enrich_meta([card])
    return enriched[0] if enriched else None


def _set_single_seed_pool(s: dict, cyanite_id: str) -> None:
    rows = catalog.find_similar(cyanite_id, limit=config.SIMILAR_LIMIT)
    s["candidate_pool"] = [{
...
def _profile_refill(s: dict) -> dict | None:
    ...
    rows = catalog.search_by_prompt(query, limit=config.PROFILE_REFILL_LIMIT)
...
def _expand_pool(s: dict) -> None:
    ...
    if len(s["liked_tracks"]) >= 2:
        rows = catalog.find_similar_multi(s["liked_tracks"], limit=config.SIMILAR_LIMIT)
    else:
        rows = catalog.find_similar(s["liked_tracks"][0], limit=config.SIMILAR_LIMIT)
    s["candidate_pool"] = [{
...
def confirm(session_id: str) -> dict:
    ...
    results = catalog.search_by_prompt(args["query"], limit=config.SEARCH_LIMIT,
                                       metadata_filter=args["metadata_filter"])
    cards = catalog.enrich_meta([_card(r["cyanite_id"], r["score"], "free_text") for r in results])
...
def _inject_surprise(s: dict) -> None:
    ...
    try:
        ...
        return
    card = catalog.enrich_meta([_card(pick["cyanite_id"], pick["score"], "surprise")])[0]
...
def explain_sounds_like_you(user_id: str, cyanite_id: str) -> dict:
    ...
    track_tags = catalog.model_tags(cyanite_id, config.EXPLAIN_TAG_MODELS)
    display = catalog.display(cyanite_id, "")
...
def explain(session_id: str, track_id: str) -> dict:
    ...
    recommended_tags = catalog.model_tags(cyanite_id, config.EXPLAIN_TAG_MODELS)
    similar_rows = catalog.find_similar(cyanite_id, limit=config.EXPLAIN_SIMILAR_LIMIT)
    display_by_id = {row["cyanite_id"]: catalog.display(row["cyanite_id"], row.get("track_id", ""))
    ...
    if explanation_example:
        display = catalog.display(explanation_example["track_id"])
        ...
        if not display.get("title") and display.get("track_id"):
            enriched = catalog.enrich_meta([display])
            if enriched:
                display = enriched[0]
    ...
    liked_tags = {}
    if explanation_example:
        liked_tags = catalog.model_tags(explanation_example["track_id"], config.EXPLAIN_TAG_MODELS)
