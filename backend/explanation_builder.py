...
def _fallback_explanation(query_card: dict,
                          recommendation_meta: dict,
                          explanation_example: dict | None,
                          recommended_track_tags: dict | None = None) -> dict:
    query = query_card.get("free_text_query") or query_card.get("interpretation_plain") or "your current search"
    ...
    if recommendation_meta.get("source_liked_track"):
        selection = f"It was selected from music close to your liked tracks by tag-based similarity{score_text}."
        ranking_basis = recommendation_meta.get("ranking_basis", "similar_score_fallback")
    else:
        selection = f"It was retrieved directly from your search brief by the catalog search{score_text}."
        ranking_basis = recommendation_meta.get("ranking_basis", "free_text_search")
    # 无 LLM 时解释也必须落到具体证据上——"为什么是这首"是这个产品的全部主张。
    traits = _trait_phrase(recommended_track_tags)
    trait_text = f" Its tags read as {traits}." if traits else ""
    evidence = [{"source": "ranking", "detail": f"ranking_basis={ranking_basis}"}]
    if traits:
        evidence.append({"source": "track_tags", "detail": traits})
    return {
        "why_text": f"This track fits your current search for {query}. {selection}{trait_text}{example_text}",
        "evidence": evidence,
    }


def _trait_phrase(track_tags: dict | None, per_model: int = 2) -> str:
    """把 model_tags 的 {"items":[{"version","tags"}]} 压成一句可读的标签短语。"""
    items = (track_tags or {}).get("items") or []
    picked = []
    for item in items:
        for tag in (item.get("tags") or [])[:per_model]:
            if isinstance(tag, str) and tag not in picked:
                picked.append(tag)
