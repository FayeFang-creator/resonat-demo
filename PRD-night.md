...
| **反馈机制** | 左右滑写记忆；踩一首剪枝，赞一首扩展回填 | **like 只记为声音证据（不搜索）；dislike 移除当前曲 → 用 liked 种子跑 similarById → 按 similarity score 回填空位** |
...
- **dislike T** → T 从当前推荐列表移除，留下一个空位。系统**这时才**对用户**已 like 的曲（种子集合）**跑 `similarById`，把找到的相似曲当候选，按 Cyanite 返回的 similarity score 排序后回填这个空位。若用户还没 like 过任何曲，就用 `free_text_backlog`（首批 freeText 召回里尚未展示的）补位。

> 关键：`similarById` **只由 dislike 触发**，且种子是用户喜欢的曲——"踩掉一首 → 用你喜欢的方向补一首"。like 不触发任何搜索，避免在用户还没踩之前就铺开不确定方向。

主办方当前提供的 API 不包含 Advanced Search，因此本版不做 `track + prompt -> score` 或 crate-scoped prompt scoring。回填排序直接使用 `similarById` / similarity API 返回的 `similar_score`，按分数从高到低选曲。

**⑥ 薄重排**
拿到回填候选后，批量拉取 liked 曲和候选曲目的 Cyanite 标签做**薄重排**——similarity score 始终是主导信号，标签软目标只做轻微调序。重排后继续等用户反馈，而不是自动无限扩展。
...
> 一句话：**用户先确认系统怎么理解需求；系统先用 freeText 给结果；用户 like 只记证据；只有 dislike 时才用 liked 种子跑 similarById，并按 similarity score 回填空位。** 解释和控制都发生在白板 + Query Card + Why this 三个可见位置上。
...
| `POST /feedback` | `{session_id, track_id, verdict}` | `{cards[], candidate_pool_size}` | like 只记 liked 种子 + 落 evidence；dislike 移除 → 对 liked 种子跑 similarById → 按 similar_score 回填 + 薄重排 |
...
candidate_pool item:
  track_id, source_liked_track, similar_score, tags?, status, ranking_basis
...
- dislike T → 从 `visible_cards` 删除 T 形成空位 → **此时**对 `liked_tracks` 跑 `similarById`（单种子或多种子）得到候选灌入 `candidate_pool` → 按 `similar_score` 选 1 首回填。
  - 若 `liked_tracks` 为空（用户还没 like 过）→ 用 `free_text_backlog` 的下一首补位。
- 回填排序（候选池非空时）：
  - 当前 API 不提供 Advanced Search 或 `track + prompt -> score`，所以按 `similar_score` 降序。
- 薄重排：`score = w_primary * primary_match + w_soft * tag_match - w_neg * neg_match`，其中 `primary_match` 是 `similar_score` / `search_score`；护栏 `w_primary > w_soft + w_neg`（**无硬过滤，负向只扣分不剔除**）。
...
4. **dislike 才触发相似搜索 + 回填**：踩掉一首 → 它从右下列表消失；系统**这时才**对用户已 like 的曲跑 `similarById`，按 `similar_score` 补进一首。（用户还没 like 过 → 用 freeText backlog 补位。）
...
1. **当前主办方提供的 API 不包含 Advanced Search。**
   - 本版不实现 `track + prompt -> score`。
   - 回填排序按 `similarById` / similarity API 返回的 `similar_score`。
2. **仍需确认 similarity 返回分数的方向和字段名。**
   - 如果分数越高越相似：按 `similar_score` 降序。
   - 如果返回的是 distance：按 distance 升序，或先归一化成 `similar_score` 再降序。
