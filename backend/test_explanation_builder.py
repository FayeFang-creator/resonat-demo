...
def test_fallback_explanation_is_grounded_and_english(monkeypatch):
    ...
    assert "tag-based similarity" in result["why_text"]
    assert result["evidence"][0]["source"] == "ranking"
    # 无 LLM 时也必须引用推荐曲的真实标签，否则「为什么是这首」就没有落点
    assert any(e["source"] == "track_tags" for e in result["evidence"]), result["evidence"]
