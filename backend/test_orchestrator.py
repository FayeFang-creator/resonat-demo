...
def _fake_seams(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(orch.catalog, "search_by_prompt", fake_search)
    monkeypatch.setattr(orch.catalog, "find_similar", fake_similar)
    monkeypatch.setattr(orch.catalog, "find_similar_multi", fake_multi)
    monkeypatch.setattr(orch.catalog, "display",
...
def test_intent_does_not_search_until_confirm(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(orch.catalog, "search_by_prompt",
...
def test_surprise_card_only_in_confirm(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(orch.catalog, "search_by_prompt", search)
...
def test_confirm_unknown_session_returns_404():
    """Cyanite 下线后 confirm 唯一的上游错误面就是未知 session。"""
    body = client.post("/intent/confirm", json={"session_id": "nope"}).json()

    assert body["detail"] == "unknown session_id"
...
def test_like_with_empty_similar_falls_back_to_backlog(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(orch.catalog, "find_similar", lambda cid, limit=20: [])
    body = client.post("/feedback", json={"session_id": sid, "track_id": "libtr_0", "verdict": "like"}).json()
    ids = [c["track_id"] for c in body["cards"]]
...
def test_explain_similar_refill_uses_session_liked_seed(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(orch.catalog, "model_tags", lambda cid, models: {"track": cid})
...
def test_explain_uses_historical_similar_intersection(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(orch.catalog, "find_similar", fake_similar)
    monkeypatch.setattr(orch.catalog, "model_tags", lambda cid, models: {"track": cid, "models": models})
    monkeypatch.setattr(orch.explanation_builder, "build_explanation", fake_build_explanation)
