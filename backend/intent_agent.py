...
def _flatten_profile(profile_md: str) -> str:
    """把记忆画像的 markdown 压成一句自然语言。

    无 key 时这段文字既进确认门（用户直接看到），又进本地检索（当检索词）。
    画像里只有 "## Your Feel" 下那句叙述是给人读的；标题、分隔线、**标签:** 统计块、
    编号的 taste timeline 都是结构化数据，原样塞进去用户会看到 "# memory · demo"、
    "Feel spectrum (by frequency): chill x16..."，检索也被这些噪声词带偏。
    """
    kept = []
    for line in profile_md.splitlines():
        line = line.strip()
        if not line or line.startswith(("#", "---", "_", "**")) or line[0].isdigit():
            continue
        kept.append(_MD.sub("", line).strip())
    # 去掉句末句号：调用方会把这段拼进 "...for {query}. It was..."，留着会出现 "cool.."
...
def _join(posts: list[dict], profile_md: str) -> str:
    ...
    profile = _flatten_profile(profile_md)
    if profile:
        parts.append(f"taste profile: {profile}")
    return "; ".join(parts)
...
if __name__ == "__main__":  # 自检：fallback + str.replace 注入 + tool-call 解析
    ...
    # 画像 markdown 必须压成一句人话：无 key 时它直接出现在确认门上，
    # 原样透出会让访客看到 "# memory · demo"、"Feel spectrum (by frequency): chill x16..."
    md = ('# memory · u\n\n## Your Feel\nYou lean **calm, warm**.\n\n---\n'
          '**Core feel (runs across rounds):** calm, warm\n\n**Taste timeline:**\n1. "x" → calm\n\n_1 likes_\n')
    assert _flatten_profile(md) == "You lean calm, warm"   # 句末句号由调用方补
    assert _flatten_profile("") == "" and _join(posts, "") == "适合健身的; 要带劲 {x}"
